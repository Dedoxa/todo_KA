...
export default class Task extends React.Component {
  ...
  resetTimer = () => {
    clearInterval(this.interval); // Останавливаем текущий таймер
    const newTime = '00:00'; // Сбрасываем время на 00:00
  
    this.setState({
      time: newTime, // Сбрасываем время на 00:00
      isRunning: false, // Устанавливаем состояние в false
    }, () => {
      // Обновляем localStorage после обновления состояния
      this.props.onRewrite(this.state.descriptionText, this.props.id, newTime); // Обновляем время в localStorage
    });
  }
  ...
  onToggleDoneAndStopTimer = () => {
    this.resetTimer()
    this.props.onToggleDone(this.props.id)
  }

  render() {
    const { id, descriptionText, onDeleted, edit, done, hidden, onEdit } = this.props
