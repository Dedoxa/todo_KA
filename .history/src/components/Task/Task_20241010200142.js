...
export default class Task extends React.Component {
  ...
  resetTimer = () => {
    ...
    this.setState(
      ...
      () => {
        this.props.onRewrite(this.state.descriptionText, this.props.id, newTime)
      }
    )
  }
      ...
      <li key={id} className={liClassNames}>
        <div className={divClasses}>
          <input
            className="toggle"
            type="checkbox"
            onMouseDown={this.onToggleDoneAndStopTimer}
            checked={checkBoxState}
            readOnly
          />
          <label>
            <span className="description" onMouseDown={this.onToggleDoneAndStopTimer}>
              {descriptionText}
            </span>
