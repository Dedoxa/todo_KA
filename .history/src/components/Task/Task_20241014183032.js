...
const Task = ({
  ...
  useEffect(() => {
    ...
    console.log('edit of this task: ', edit)
  ...
  const getPreviousDescriptionText = () => {
    ...
    console.log('parsedStorage: ', parsedStorage, id, parsedStorage[id])
    const idx = parsedStorage.findIndex((element) => element.id === id)
    const prevDescriptionText = parsedStorage[idx].descriptionText
    setOverviewText(prevDescriptionText)
  }

  const handleKeyDown = (e) => {
    console.log('e.key: ' + e.key + ', edit: ' + edit)
  ...
  const handleClickOutside = (e) => {
    console.log('edit: ' + edit + ', taskRef.current: ' + taskRef.current)
    if (edit && taskRef.current && !taskRef.current.contains(e.target)) {
      console.log('кликаутсайдишь')
      getPreviousDescriptionText()
      onEdit()
    }
  }
  ...
  return (
    <li key={id} className={edit ? 'editing' : ''} ref={taskRef}>
