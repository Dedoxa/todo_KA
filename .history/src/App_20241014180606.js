...
export default class App extends React.Component {
  ...
  startId = this.state.tasks.length > 0 ? this.state.tasks.length - 1 : 0

  componentDidMount() {
    const savedTasks = localStorage.getItem('tasks')
    console.log(JSON.parse(savedTasks), JSON.parse(savedTasks).length)
    JSON.parse(savedTasks).map((element) => console.log(element))
