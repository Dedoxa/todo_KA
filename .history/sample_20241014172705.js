...
const Task = ({
  ...
  const [timeFromCreation, setTimeFromCreation] = useState(
    formatDistanceToNow(dateOfCreation, { includeSeconds: true })
  )
  ...
  useEffect(() => {
    ...
    setTimerInterval(
      setInterval(() => {
        setTimeFromCreation(formatDistanceToNow(dateOfCreation, { includeSeconds: true }))
      }, 1000)
    )
  ...
  const handleClickOutside = (e) => {
    // !!! Проверяем, находится ли клик за пределами компонента !!!
  ...
  if (done) {
    ...
    divClasses = divClasses.replace(' completed', '')
    checkBoxState = false
  }
  ...
  if (hidden) {
    ...
    divClasses = divClasses.replace(' hidden', '')
    editFormClasses = editFormClasses.replace(' hidden', '')
  }
